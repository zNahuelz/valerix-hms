...
new class extends Component
{
    ...
    {
        return $this->view()
            ->layout('layouts::dashboard', ['heading' => __('nurse.index')])
            ->title(__('views.nurse.index'));
    }
};
?>
...
<div>
    <div class="flex flex-col md:flex-row w-full items-stretch md:items-center justify-between gap-4 mb-2">
        @canany(['sys.admin', 'nurse.create'])
            <flux:button variant="primary" icon="plus" wire:navigate href="{{ route('nurse.create') }}"
        ...
        <flux:table.rows>
            @forelse($this->nurses as $nurse)
                <flux:table.row class="hover:bg-accent-content/10">
                    <flux:table.cell>{{ $nurse->id }}</flux:table.cell>
                    <flux:table.cell>{{ $nurse->names }}</flux:table.cell>
                    <flux:table.cell>{{ $nurse->paternal_surname }}</flux:table.cell>
                    <flux:table.cell>{{ $nurse->dni }}</flux:table.cell>
                    <flux:table.cell>{{ $nurse->phone ?? __('common.null') }}</flux:table.cell>
                    <flux:table.cell>
                        <flux:badge color="{{ $nurse->deleted_at ? 'red' : 'green' }}" size="sm" inset="top bottom">
                            {{ $nurse->deleted_at ? __('common.disabled_entity') : __('common.enabled_entity') }}
                        </flux:badge>
                    </flux:table.cell>
                    <flux:table.cell>{{ $nurse->updated_at->timezone('America/Lima')->format('d/m/Y g:i A') }}
                    </flux:table.cell>
                    <flux:table.cell>
                        <flux:button.group>
                            @if(auth()->id() !== $nurse->user_id)
                                @canany(['sys.admin', 'nurse.edit', 'nurse.delete', 'nurse.restore'])
                                ...
                                @endcanany
                                @canany(['sys.admin', 'nurse.detail'])
                                    <flux:button variant="ghost" size="sm" icon="ellipsis-horizontal" inset="top bottom"
                                                 title="{{ __('common.details') }}"
                                                 href="{{ route('nurse.detail', ['nurseId' => $nurse->id]) }}"
                                                 wire:navigate>
                                    </flux:button>
                                @endcanany
                            @endif
            ...
            @empty
                <flux:table.row>
                    <flux:table.cell colspan="8" class="text-center text-lg md:text-xl font-light">
                        {{ __('nurse.errors.empty_set') }}
                    </flux:table.cell>
                </flux:table.row>
            @endforelse
        </flux:table.rows>
    </flux:table>
    <x-shared.pagination :paginator="$this->nurses"></x-shared.pagination>
