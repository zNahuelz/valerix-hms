...
new class extends Component {
    ...
    {
        return $this->view()
            ->layout('layouts::dashboard', ['heading' => __('voucher-type.index')])
            ->title(__('views.voucher_type.index'));
    }
};
?>
...
<div>
    <div class="flex flex-col md:flex-row w-full items-stretch md:items-center justify-between gap-4 mb-2">
        @canany(['sys.admin', 'voucherType.create'])
            <flux:button variant="primary" icon="plus" wire:navigate href="{{ route('voucherType.create') }}"
    ...
    <flux:table>
        ...
        <flux:table.rows>
            @forelse($this->voucherTypes as $voucherType)
                <flux:table.row class="hover:bg-accent-content/10">
                    <flux:table.cell>{{ $voucherType->id }}</flux:table.cell>
                    <flux:table.cell>{{ $voucherType->name }}</flux:table.cell>
                    <flux:table.cell>{{ $voucherType->created_at->timezone('America/Lima')->format('d/m/Y g:i A') }}</flux:table.cell>
                    <flux:table.cell>{{ $voucherType->updated_at->timezone('America/Lima')->format('d/m/Y g:i A') }}</flux:table.cell>
                    <flux:table.cell>
                        @canany(['sys.admin', 'voucherType.edit'])
            ...
            @empty
                <flux:table.row>
                    <flux:table.cell colspan="5" class="text-center text-lg md:text-xl font-light">
                        {{ __('voucher-type.errors.empty_set') }}
                    </flux:table.cell>
                </flux:table.row>
            @endforelse
        </flux:table.rows>
    </flux:table>
    <x-shared.pagination :paginator="$this->voucherTypes"></x-shared.pagination>
</div>
