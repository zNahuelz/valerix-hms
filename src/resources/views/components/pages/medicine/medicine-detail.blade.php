    ...
    <div class="w-full md:max-w-2xl">
        ...
        <flux:fieldset class="grid grid-cols-1 md:grid-cols-2 gap-2">
            <flux:field>
                ...
                <flux:input readonly value="{{ $medicine->name }}" type="text"/>
            </flux:field>
            <flux:field>
                <flux:label>{{ __('common.barcode') }}</flux:label>
                <flux:input readonly value="{{ $medicine->barcode }}" type="text"/>
            </flux:field>
            <div class="col-span-full">
                <flux:field>
                    <flux:label>{{ __('common.composition') }}</flux:label>
                    <flux:input readonly value="{{ $medicine->composition }}" type="text"/>
                </flux:field>
            </div>
            <div class="col-span-full">
                <flux:input readonly
                            value="{{ $medicine->description != '' ? $medicine->description : __('common.null') }}"
            ...
            <div class="col-span-full">
                <flux:field>
                    ...
                    <flux:input.group>
                        <flux:input readonly value="{{ $medicine->presentation->description  }}"
                                    type="text"/>
                        <flux:button type="button" variant="primary" color="cyan"
                                     icon="ellipsis-horizontal" wire:navigate
                                     href="{{route('presentation.detail',['presentationId' => $medicine->presentation->id])}}">
                        </flux:button>
                    </flux:input.group>
                </flux:field>
            </div>
            <flux:input readonly value="{{ $medicine->createdBy->username ?? __('common.inserted_by_null') }}"
                        label="{{ __('common.created_by') }}" type="text"/>
            <flux:input readonly value="{{ $medicine->updatedBy->username ?? __('common.inserted_by_null') }}"
